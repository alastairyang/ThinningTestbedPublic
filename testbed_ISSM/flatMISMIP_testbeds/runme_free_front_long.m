...
%%%%%%%%%%%%%% Instruction %%%%%%%%%%%%%%
% You can change line 68 to select the testbeds.
% You can change line 89 to select the simulation tasks.
%%%%%%%%%%%%%% End of Instruction %%%%%%%%%%%%%%
...
for jj = [18] % Consult "mdvar_combs" for the model index (the row number)
    ...
    for steps = 13
        ...
        if perform(org, pulse_gauss_mu_title)% {{{1 STEP 9: Basal perturbation: pulse gaussian patch + overburden pressure
            md = loadmodel(org, 'Transient_ExtraInfo');
        ...
        if perform(org, diffu_gauss_mu_plastic_title)% {{{1 STEP 12: Basal perturbation: diffused pulse gaussian patch + effective pressure + plastic bed rheology
            md = loadmodel(org, 'Transient_ExtraInfo');
            m_plastic = 5;
