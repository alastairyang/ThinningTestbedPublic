...
gl_corrs = cell(2,n_simu);
front_corrs = cell(2,n_simu);
dist_to_front = cell(2,n_simu);
gl_decay_length = zeros(2,n_simu);
front_decay_length = zeros(2,n_simu);
gl_corrcoef_mean = zeros(2,n_simu);
front_corrcoef_mean = zeros(2,n_simu);
...
for q = 1:length(GLs) % shallow, deep
    ...
    for j = 1:n_simu
        ...
        % save to cells
        gl_corrs{q,j} = gl_corr_mid;
        front_corrs{q,j} = front_corr_mid;
...
for j = 1:n_simu
    ...
    % plot deep
    scatter(dist_to_front{2,j}/1000, front_corrs{2,j},...
        '+','MarkerFaceColor','r','MarkerEdgeColor','r'); hold on;
    scatter(dist_to_front{2,j}/1000, gl_corrs{2,j},...
...
exportgraphics(gcf,['plots/correlation_',md_type,'_deep.png'],"Resolution",500)
